    ...
    <div class="childMenu">
      <template v-for ="(item, i) in childMenuList">
        <div class="childMenuItem" :key="i" >
          <p class="title" @click="titleClick(item.title)">{{item.title}}</p>
...
export default {
  ...
  methods: {
    ...
    titleClick(name) {
      switch (name) {
        case 'Products':
          this.$router.push({ path : '/Detail', query: { name: 'SW320-330W-M' } })
          break;
      ...
      switch (item.title) {
        case 'Products':
          if(name !== 'PV Accessories') {
            ...
            this.$router.push({ path : '/Detail', query: { name: 'Folding Solar Panels' } })
          }
          break;
        case 'Global Supply Chain':
          this.$router.push({ path : '/Global' })
